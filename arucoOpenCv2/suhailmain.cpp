...
Vec<double,3> averager[10] = {0,0,0};
int cycle = 0;
Vec<double,3> fivePointAverage(Vec<double,3> input){
...
int startWebcamMonitoring(const Mat& cameraMatrix, const Mat& distanceCoefficients, float arucoSquareDimensions){
	...
	while (true) {
		...
		for (int i = 0; i < markerIds.size(); i++) {
			aruco::drawAxis(frame, cameraMatrix, distanceCoefficients, rotationVectors[i], translationVectors[i], 0.0235f);
