...
// Initialization
    ...
    double degree = 90.0;
    do {
        printf("Input Distance & Angle:\n");
        int angle, dist;
        alGetSourcei(source, AL_SOURCE_STATE, &state);
        scanf("%d %d", &dist, &angle);

        pos[0] = (ALfloat) dist*cos((angle+90)*PI/180);    //x
        pos[2] = (ALfloat) -1*dist*sin((angle+90)*PI/180);    //y
	...
	int iii = 0;
	time_t end_time = time(NULL) + 6;
	time_t cur_time = time(NULL);
	while((iii < 5) && (cur_time < end_time))
        {
                alSourcePlay(source);
                alGetSourcei(source, AL_SOURCE_STATE, &state);
                while((state == AL_PLAYING) && (error == AL_NO_ERROR) && (cur_time < end_time))
                {
		    cur_time = time(NULL);
                    alGetSourcei(source, AL_SOURCE_STATE, &state);
		    error = alGetError();
